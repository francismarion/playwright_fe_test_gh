...
```
Error: These items did not switch to "Remove": Sauce Labs Bike Light, Sauce Labs Backpack, Sauce Labs Bolt T-Shirt, Sauce Labs Onesie, Sauce Labs Fleece Jacket, Test.allTheThings() T-Shirt (Red)
```
...
```ts
  ...
  52 | 
  53 |   for (const item of expectedItems) {
  54 |     const button = this.productNameLocator(item);
  55 | 
  56 |     // Wait for the button to be attached before reading text
  57 |     await expect(button).toBeVisible();
  58 | 
  59 |     const text = (await button.innerText()).trim();
  60 | 
  61 |     if (text !== 'Remove') {
  62 |       notChanged.push(item);
  63 |     }
  64 |   }
  65 | 
