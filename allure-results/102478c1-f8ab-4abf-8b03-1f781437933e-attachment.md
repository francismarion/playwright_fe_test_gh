...
Call log:
  - waiting for locator('.inventory_item button').nth(2).locator('..').locator('.inventory_item_name')
...
```ts
  ...
  51 |   const buttons = this.page.locator('.inventory_item button');
  52 |   const count = await buttons.count();
  53 |   const notChanged: string[] = [];
  54 | 
  55 |   for (let i = 0; i < count; i++) {
  56 |     const button = buttons.nth(i);
  57 |     const text = (await button.innerText()).trim();
  58 | 
  59 |     if (text === 'Add to cart') {
  60 |       // grab the product name text from the same card
> 61 |       const productName = await button.locator('xpath=..').locator('.inventory_item_name').innerText();
     |                                                                                            ^ Error: locator.innerText: Test timeout of 30000ms exceeded.
  62 |       notChanged.push(productName);
  63 |     }
  64 |   }
  65 | 
